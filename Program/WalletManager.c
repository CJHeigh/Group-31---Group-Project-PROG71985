...
{
	// Display and print all wallet ID's, usernames, and btc amounts // COMPLETED NOT TESTED

	// Minor efficiency problem, if the last piece of data is in wallet number 10 this function will contine to search,
	// It will end at the final wallet position, this is inefficient but nto sure how to get around it since there is
	// the potential that the entire wallet could be empty other than the last element. With only 60 entries this is
	// not very relevent but if this database had a million entries this method would be very slow.

	printf("\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n\n");

	for (int walletNum = 0; walletNum < DATABASE_SIZE; walletNum++)
	{
		printf("Wallet ID #%d ~~ Username -> %s ~~ Bitcoin amount -> %f\n", walletDatabase[walletNum]->walletID, walletDatabase[walletNum]->walletUsername, walletDatabase[walletNum]->btcAmount);
	}
